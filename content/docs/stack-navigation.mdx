...
category: Navigation
status: built
summary: "The navigation model that runs every mobile app you've ever used, and the one web engineers reinvent badly. Here's the push/pop stack from first principles to…"
tags: [navigation, stack]
---
...
Rule: **Navigator 1.0 for a phone app's simple drill-downs; 2.0 / go_router the moment URLs, deep links, or web enter the picture.**
...
**The title is the label.** Each screen's nav title must be meaningful out of context ("Order 42 tracking," not "Details"), because it's what gets announced and what appears in the back button's "back to ___" affordance.
